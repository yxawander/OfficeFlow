...
public class DocumentInitRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        log.info("Scanning document directory: {}", documentPath);

        File dir = new File(documentPath);
        if (!dir.exists() || !dir.isDirectory()) {
            log.warn("Document directory does not exist or is not a directory: {}", documentPath);
            return;
        }
        ...
        if (docFiles == null || docFiles.length == 0) {
            log.info("No document files (pdf/txt/md) found in {}", documentPath);
            return;
        }
